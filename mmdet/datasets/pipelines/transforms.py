...
class RandomFlip(object):
    ...
    def __call__(self, results):
        image = results['img']
        detections = results['gt_bboxes']
        gt_labels = results['gt_labels']
        flip = False
        if np.random.uniform() > self.flip_ratio:
            image[:] = image[:, ::-1, :]
            width = image.shape[1]
            if gt_labels[0]!=0:
        ...
        if rot_num > 0:
            ...
            if gt_labels[0]!=0:
                for _ in range(rot_num):
                    detections = self.rot(image.shape[0], detections)
            flip = True
        results['flip'] = flip
        results['img'] = image
        results['gt_bboxes'] = detections
...
class Augment(object):
    def __init__(self,eig_val,eig_vec,ratio=0.5,data_rng=np.random.RandomState(123)):
        self.eig_val = np.array(eig_val,dtype=np.float32)
    ...
    def color_jittering_(self,data_rng, image):
    ...
    def __call__(self, results):
        ...
        self.color_jittering_(self.data_rng, image)
        # if np.random.uniform() > self.ratio:
        self.lighting_(self.data_rng, image, 0.1, self.eig_val, self.eig_vec)
        results['img'] = image
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += '(eig_val={}, eig_vec={})'.format(
            self.eig_val, self.eig_vec)
        return repr_str
...
class Normalize(object):
    ...
    def __call__(self, results):
        results['img'] =mmcv.imnormalize(results['img'], self.mean, self.std,
                                          self.to_rgb)
        # results['img'] = self.normalize_(results['img'], self.mean, self.std)
