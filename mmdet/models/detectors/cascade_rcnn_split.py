...
class SCascadeRCNN(BaseDetector, RPNTestMixin):
    ...
    def __init__(self,
        ...
        super(SCascadeRCNN, self).__init__()

        self.num_stages = num_stages
        self.backbone = builder.build_backbone(backbone)

        if neck is not None:
            self.neck = builder.build_neck(neck[0])
            self.m2neck = builder.build_neck(neck[1])
    ...
    def init_weights(self, pretrained=None):
        super(SCascadeRCNN, self).init_weights(pretrained)
        self.backbone.init_weights(pretrained=pretrained)
        if self.with_neck:
            if isinstance(self.neck, nn.Sequential):
                for m in self.neck:
                    m.init_weights()
            elif isinstance(self.neck, nn.ModuleList):
                for i in range(self.num_stages):
                    self.neck[i].init_weights()
            else:
    ...
    def extract_feat(self, img):
        ...
        out = []
        if self.with_neck:
            out.append(self.neck(x))
            out.append(self.m2neck(x[1:]))
        return out
    ...
    def forward_train(self,
        ...
        """
        ex = self.extract_feat(img)
        assert len(ex)==2
        rpn_x = ex[0]
        bbox_x = tuple([rpn_x[0]]+list(ex[1][:4]))

        losses = dict()

        if self.with_rpn:
            rpn_outs = self.rpn_head(rpn_x)
        ...
        for i in range(self.num_stages):
            ...
            if self.with_bbox or self.with_mask:
                ...
                for j in range(num_imgs):
                    ...
                    sampling_result = bbox_sampler.sample(
                        ...
                        gt_bboxes[j],
                        gt_labels[j],
                        feats=[lvl_feat[j][None] for lvl_feat in bbox_x])
            ...
            bbox_feats = bbox_roi_extractor(bbox_x[:bbox_roi_extractor.num_inputs],
                                            rois)
            if self.with_shared_head:
                bbox_feats = self.shared_head(bbox_feats)
            ...
            if self.with_mask:
                if not self.share_roi_extractor:
                    ...
                    mask_feats = mask_roi_extractor(
                        bbox_x[:mask_roi_extractor.num_inputs], pos_rois)
                    if self.with_shared_head:
                        mask_feats = self.shared_head(mask_feats)
                else:
    ...
    def simple_test(self, img, img_meta, proposals=None, rescale=False):
        ...
        proposal_list = self.simple_test_rpn(
            rpn_x, img_meta, self.test_cfg.rpn) if proposals is None else proposals
        ...
        for i in range(self.num_stages):
            ...
            bbox_feats = bbox_roi_extractor(
                bbox_x[:len(bbox_roi_extractor.featmap_strides)], rois)
            if self.with_shared_head:
                bbox_feats = self.shared_head(bbox_feats)
            ...
            if self.test_cfg.keep_all_stages:
                ...
                if self.with_mask:
                    ...
                    if det_bboxes.shape[0] == 0:
                        ...
                        mask_feats = mask_roi_extractor(
                            bbox_x[:len(mask_roi_extractor.featmap_strides)],
                            mask_rois)
                        if self.with_shared_head:
        ...
        # if results[0].shape[0] > 100:
        #     results = torch.from_numpy(results[0])
        #     s, ind = torch.topk(results[:, 4], results)
        #     results = results[ind].numpy()
        # elif results[0].shape[0] == 0:
        #     results = np.zeros([100, 5], dtype=np.float32)
        #
        # else:
        #     results = results[0]
        #     results = np.vstack([results, np.zeros([100 - results.shape[0], 5], dtype=np.float32)])
        return results
    ...
    def show_result(self, data, result, **kwargs):
        ...
        super(SCascadeRCNN, self).show_result(data, result, **kwargs)
